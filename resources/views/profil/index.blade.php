...
@section('content')
    <div class="bg-blue-600 text-white py-12">
        <div class="container mx-auto px-4">
            <h1 class="text-4xl font-bold">Tentang {{ $profil->nama_sekolah ?? 'Sekolah' }}</h1>
        </div>
    </div>
                ...
                @if ($profil)
                    <div class="bg-white rounded-lg shadow p-8 mb-8">
                        ...
                        @if ($profil->foto_sekolah)
                            <img src="{{ asset('storage/' . $profil->foto_sekolah) }}" alt="Sekolah"
                                class="w-full h-64 object-cover rounded-lg mb-6">
                        @endif
                        <p class="text-gray-700 leading-relaxed mb-4">{{ $profil->tentang }}</p>

                        <div class="grid grid-cols-2 gap-4 mt-6">
                            <div class="border-l-4 border-blue-600 pl-4">
                                <p class="text-sm text-gray-600">NPSN</p>
                                <p class="font-semibold text-gray-800">{{ $profil->npsn ?? '-' }}</p>
                            </div>
                            <div class="border-l-4 border-green-600 pl-4">
                                <p class="text-sm text-gray-600">Akreditasi</p>
                                <p class="font-semibold text-gray-800">{{ $profil->akreditasi ?? '-' }}</p>
                            </div>
                            <div class="border-l-4 border-purple-600 pl-4">
                                <p class="text-sm text-gray-600">Kepala Sekolah</p>
                                <p class="font-semibold text-gray-800">{{ $profil->kepala_sekolah ?? '-' }}</p>
                            </div>
            ...
            <div>
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <ul class="space-y-2">
                        ...
                        <li><a href="{{ route('profil.ekstrakurikuler') }}"
