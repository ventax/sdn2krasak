...
@section('content')
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $stats['total_ppdb'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $stats['ppdb_pending'] }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    @forelse($berita_terbaru as $berita)
                        <div class="flex items-start space-x-3 pb-4 border-b border-gray-100 last:border-0">
                            ...
                            <span
                                class="text-xs px-2 py-1 rounded {{ $berita->is_published ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
        ...
        <div class="bg-white rounded-lg shadow">
            ...
            <div class="p-6">
                <div class="space-y-4">
                    @forelse($ppdb_terbaru as $pendaftar)
                        <div class="flex items-start justify-between pb-4 border-b border-gray-100 last:border-0">
                            <div>
                                <h4 class="font-medium text-gray-900">{{ $pendaftar->nama_lengkap }}</h4>
                                <p class="text-sm text-gray-500 mt-1">{{ $pendaftar->no_pendaftaran }}</p>
                                <p class="text-xs text-gray-400">{{ $pendaftar->created_at->format('d M Y H:i') }}</p>
                            </div>
                            <span
